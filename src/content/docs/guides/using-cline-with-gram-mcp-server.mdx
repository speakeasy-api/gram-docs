---
title: "Using Cline with Gram MCP servers"
description: "Learn how to use Cline with Gram MCP servers."
sidebar:
  order: 6
---

Cline is a [VS Code extension](https://marketplace.visualstudio.com/items?itemName=saoudrizwan.claude-dev) that embeds autonomous coding agents right inside your editor. Once installed, the extension has full read-write access to your open workspace, letting you chat with the agent, generate code, run tests, and refactor files within your editor.

When paired with [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) servers, you can connect it to your APIs, databases, and infrastructure services, granting Cline contextual access to your tools and data.

In the guide that follows, we'll walk through connecting Cline to a [Gram-hosted MCP server](/blog/the-easiest-way-to-host-mcp-servers) using our Push Advisor example from the [Gram concepts guide](/blog/gram-concepts). You'll learn how to set up the connection, test it, and use natural language to perform vibe checks before you push code.
...
- The [Cline extension](https://marketplace.visualstudio.com/items?itemName=saoudrizwan.claude-dev) installed.
- A paid Cline subscription or an Anthropic, OpenAI, or any other LLM provider API key supported by Cline.

## Creating a Gram MCP server

If you already have a Gram MCP server configured, you can skip to the [Connect Cline to your MCP server](#connect-cline-to-your-mcp-server) section. Otherwise, let's set up the Push Advisor example now.
...
## Connect Cline to your MCP server

Now we'll connect Cline to your newly created MCP server.

### Add the server to Cline

Cline uses a configuration file to manage MCP servers. To add a server to Cline, open the Cline panel in VS Code, click the **MCP Servers** icon in the Cline chat interface, and select the **Remote Servers** tab. Click **Edit Configuration** to open the `cline_mcp_settings.json` file.

![Screenshot showing the Cline MCP settings button](/img/guides/mcp-installing-cline/cline-mcp-button.png)

Add your MCP server configuration to the file:
...
You'll need an API key to use an authenticated server. [Generate an API key](/concepts/api-keys) in the **Settings** tab and replace `<your-key-here>` with your actual Gram API key.
...
You can choose to approve the tool call once, or always _auto-approve_ Cline's requests, which will disable future prompts for tool calls. Once approved, Cline will make the API call and respond with the vibe check results.
