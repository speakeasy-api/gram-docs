...
You can also view more details about the server's tools by using `Ctrl + T` to toggle the tool details.
...
<video width="600" controls>
  <source src="/img/guides/mcp-gemini/gemini-cli-showcase.mp4" type="video/mp4" />
...
You can choose to approve the tool call once, or always allow this tool, which will disable future prompts for this tool. Once approved, Gemini will make the API call and respond with the vibe check results:
...
If Gemini CLI can't find your server, press `Ctrl + O` to view the MCP Debug console.
...
1. Verify your Gram API key in the dashboard under **Settings > API Keys**
2. Check that your environment variables are set correctly in Gram
3. Ensure the Push Advisor API base URL is accessible
