...
In your default environment variables, set the `TASKBOARD_SERVER_URL` to the ngrok URL (for example, `TASKBOARD_SERVER_URL=https://abc123.ngrok.io`).
...
The chat service directory already exists in the project root. Navigate to it:
...
The `mcp-agent-service` directory contains a `main.py` file with a FastAPI service that uses `mcp-agent` to connect to the Gram MCP server. The structure looks like this:
...
Next, create a `.env` file in the `mcp-agent-service` directory to store environment variables. You can get your OpenAI API key from the [OpenAI dashboard](https://platform.openai.com/account/api-keys).
...
The `mcp_agent.config.yaml` file configures the chat agent. Update it with your own Gram MCP server URL. This file tells the agent how to connect to the Gram MCP server and how to use the OpenAI API. For more information on the configuration, see the [`mcp-agent` documentation](https://github.com/lastmile-ai/mcp-agent).
...
mcp:
  servers:
    GramTaskboard:  # This matches the server name from the MCP Config section above
      ...
      args: [
        "mcp-remote",
        "https://app.getgram.ai/mcp/your-gram-mcp-server-url",  # Your Gram MCP server URL
        "--header",
...
The TaskBoard example includes a chat component implementation. The repository contains a simple popup component that floats over the main interface.

```tsx src/components/ChatPopup.tsx
...
The Next.js API route at `src/app/api/chat/route.ts` acts as a secure proxy between the frontend chat component and the Python microservice. This route handles user authentication and forwards the `userID` to the agent:
...
The chat popup is already integrated into the TaskBoard dashboard, where it floats over the Kanban board without getting in the way.
...
The top-level `docker-compose.yml` file already includes the chat service configuration:
...
Now create a `.env` file in the project root directory (alongside `docker-compose.yml`) to include the chat service URL, the MCP server URL, and the OpenAI API key. Docker will automatically pick up these variables and use them in both services.
...
If you haven't already started ngrok from the earlier setup, expose the TaskBoard API so Gram can access it:

```bash
# In a separate terminal, start the ngrok tunnel (only if not already running)
ngrok http 3000

# Copy the public URL (like https://abc123.ngrok.io) 
# and update your .env file and Gram environment variables with this URL
```

**Note**: If you restart ngrok, you'll get a new URL and will need to update both your `.env` file and the Gram environment variables in your MCP server configuration.
