...
When paired with [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) servers, you can connect it to your APIs, databases, and infrastructure services, granting Roo Code contextual access to your tools and data.

In the guide that follows, we'll walk through connecting Roo Code to a [Gram-hosted MCP server](/blog/the-easiest-way-to-host-mcp-servers) using our Push Advisor example from the [Gram concepts guide](/blog/gram-concepts). You'll learn how to set up the connection, test it, and use natural language to perform vibe checks before you vibe code.
...
If you already have a Gram MCP server configured, you can skip to the [Connect Roo Code to your MCP server](#connect-roo-code-to-your-mcp-server) section. Otherwise, let's set up the Push Advisor example now.
...
Add the following to the `mcp_settings.json` file:
