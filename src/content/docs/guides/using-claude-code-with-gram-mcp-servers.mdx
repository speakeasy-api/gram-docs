...
![Claude Code intro](/img/guides/claude-code/claude-code-intro.png)
...
In this guide, we'll show you how to connect Claude Code to a [Gram-hosted MCP server](/blog/the-easiest-way-to-host-mcp-servers) using our Push Advisor API example from the [Gram concepts guide](/blog/gram-concepts). You'll learn how to set up the connection, test it, and use natural language to perform vibe checks before you vibe code.
...
This setup enables asking Claude Code if the vibes are good to push code to production right from the terminal, with Claude Code automatically calling the appropriate API endpoints through the MCP server.
...
Before starting, install **Claude Code** ([install from Anthropic](https://docs.anthropic.com/en/docs/claude-code)) and create a **Gram account** (sign up at [getgram.ai](https://getgram.ai)).

Verify Claude Code is installed correctly:

```bash
claude --help
```

You should see Claude Code's available commands and options. You'll need to [authenticate with Claude Code to use it](https://docs.anthropic.com/en/docs/claude-code/setup).
...
Gram provides a guided setup flow to create your toolset and MCP server. Go to the [Gram dashboard](https://getgram.ai), click the **New Project** button.

![New project in Gram](/img/guides/claude-code/gram-new-project.png)

Enter a project name and click **Submit**. You'll then be guided through the following steps:
...
![Upload OpenAPI Spec](/img/guides/claude-code/upload-openapi-spec.png)
...
![Push Advisor create toolset](/img/guides/claude-code/create-toolset.png)
...
![Configure MCP](/img/guides/claude-code/configure-mcp.png)

Your Push Advisor Toolset will be created, and you can find it in the **Toolsets** tab.

![Push Advisor Toolset](/img/guides/claude-code/toolset-created.png)

### Configuring environment variables

[Environments](../concepts/environments) store API keys and configuration separate from your toolset logic.

Go to the **Environments** tab and click the "Default" environment. Click on **Edit** and then **Fill for Toolset** and select your "Push Advisor" toolset to automatically populate the required variables.

![Fill environment variables](/img/guides/claude-code/fill-env-vars-toolset.png)

Set the `<API_name>_SERVER_URL` environment variable to `https://canpushtoprod.abdulbaaridavids04.workers.dev`. Click **Update** and then **Save**.

![Set server URL](/img/guides/claude-code/set-server-url.png)

### Publishing your MCP server

Finally, let's make the toolset available as an MCP server.
...
![MCP Details](/img/guides/claude-code/mcp-details.png)

Scroll down to the **MCP Config** section and copy the Public Server configuration.

![Push Advisor MCP Server](/img/guides/claude-code/copy-mcp-config.png)
...
![Claude Code MCP Install](/img/guides/claude-code/claude-code-mcp-install.png)
...
![Claude Code Tool Permission](/img/guides/claude-code/claude-code-tool-permission.png)
...
![Claude Code Push Request](/img/guides/claude-code/claude-code-push-request.png)
...
If Claude Code isn't calling the tools, test the MCP server in the **Gram Playground** first, check that the toolset includes the tools to use, and verify the environment is correctly configured with required variables.
