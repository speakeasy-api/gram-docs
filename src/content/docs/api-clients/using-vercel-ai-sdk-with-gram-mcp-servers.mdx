---
title: "Using Vercel AI SDK with Gram-hosted MCP servers"
description: "Learn how to connect Vercel AI SDK to Gram-hosted MCP servers, giving your AI applications access to your APIs and tools."
sidebar:
  order: 3
---

The [Vercel AI SDK](https://ai-sdk.dev/) supports remote MCP servers through their experimental MCP client feature. This allows you to give AI models direct access to your tools and APIs by connecting to [Gram-hosted MCP servers](/gram-quickstart).

This guide will show you how to connect the Vercel AI SDK to a Gram-hosted MCP server using an example [Push Advisor API](https://github.com/ritza-co/gram-examples/blob/main/push-advisor-api/openapi.yaml). You'll learn how to create an MCP server from an OpenAPI document, set up the connection, configure authentication, and use natural language to query the example API.
...
If you already have a Gram MCP server configured, you can skip to [connecting Vercel AI SDK to your Gram-hosted MCP server](#connecting-vercel-ai-sdk-to-your-gram-hosted-mcp-server). For an in-depth guide to how Gram works and more details on creating a Gram-hosted MCP server, check out our [introduction to Gram](/introduction).
...
For this guide, we'll use the public server URL format:
...
## Connecting Vercel AI SDK to your Gram-hosted MCP server
...
- `StreamableHTTPClientTransport` - Uses the Streamable HTTP transport (as opposed to SSE or STDIO)
- `new URL(...)` - Your Gram-hosted MCP server URL.
- `headers` - Optional HTTP headers for authentication (passed as second parameter to the transport constructor).

## Tool filtering and schema approaches

The Vercel AI SDK supports two approaches for working with MCP tools: schema discovery and schema definition.

### Schema discovery (recommended for Gram)

The simplest approach where all tools offered by the server are listed automatically:
...
When you define `schemas`, the client will only pull the explicitly defined tools, even if the server offers additional tools.
...
When using tools with `generateText()`, the default behavior is:
1. Model calls the appropriate tool(s)
2. Model receives tool results
3. **Model stops without generating text response**
...
1. **Step 1**: Execute tool calls
2. **Step 2**: Generate text response based on tool results
...
- **Vercel AI SDK**: Uses `experimental_createMCPClient` with Streamable HTTP/stdio transports
- **OpenAI**: Uses `tools` array with `type: "mcp"` in Responses API
- **Anthropic**: Uses `mcp_servers` parameter in Messages API

### Authentication

- **Vercel AI SDK**: HTTP headers in transport configuration
- **OpenAI**: Headers object in tool configuration
- **Anthropic**: Authorization token parameter

### Tool management

- **Vercel AI SDK**: Schema discovery or definition at client level, `activeTools` for filtering
- **OpenAI**: Tool filtering via `allowed_tools` parameter
- **Anthropic**: Tool configuration object with `allowed_tools` array

### Model flexibility

- **Vercel AI SDK**: Works with any AI provider (OpenAI, Anthropic, Google, etc.)
- **OpenAI**: Only works with OpenAI models
- **Anthropic**: Only works with Anthropic models

### Transport options

- **Vercel AI SDK**: Streamable HTTP, stdio, or custom transports
- **OpenAI**: Direct HTTP/HTTPS connections
- **Anthropic**: URL-based HTTP connections

### Response handling

- **Vercel AI SDK**: Unified tool calls/results across all providers
- **OpenAI**: `mcp_call` and `mcp_list_tools` items
- **Anthropic**: `mcp_tool_use` and `mcp_tool_result` blocks
...
You now have the Vercel AI SDK connected to your Gram-hosted MCP server, giving your AI applications access to your custom APIs and tools with the flexibility to use any AI provider.
