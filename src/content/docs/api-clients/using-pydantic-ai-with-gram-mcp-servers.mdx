...
- `url` is your Gram-hosted MCP server URL.
...
- **OpenAI** uses a `tools` array with the `type: "mcp"` in the Responses API.
...
- **Others** include direct API usage without additional abstractions.

### Transport support

- **Pydantic AI** uses Streamable HTTP transport for remote servers.
- **LangChain** uses both `streamable_http` and `stdio` transports
- **The Vercel AI SDK** uses SSE, stdio, or custom transports.
