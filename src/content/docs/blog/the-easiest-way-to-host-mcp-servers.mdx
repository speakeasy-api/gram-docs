...
* **Local** - a developer downloads an MCP server to their developer machine, and configures and MCP client, like Claude Desktop or Cursor, to be able to use the tools provided in that server
...
At the moment, unintuitively, MCP Servers are usually **local** -- hosted or managed on the **API consumer** side. If you're a developer and you want to interact with your payments platform or project management application through an MCP client, you first download the MCP server, run it locally, and hook it up to your MCP client (for example, Claude Desktop, or Cursor) Then you can chat with your API and say things like "show me all my tasks due tomorrow" or "please send reminders to everyone with invoices due this week". This is often the lowest-friction way to play with MCP, but it come with many downsides like replicated work (everyone at the company needs to run the same local Docker container or similar) and lack of control (there's no centralized place to monitor and control MCP-related requests).
