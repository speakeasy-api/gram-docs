...
And that's not all. GramAI also lets you use your MCP server, typically in two ways as a **Public server** or **Authenticated server**. You can access the generated configuration by navigating to **MCP** on the sidebar and selecting **MCP Config** on your hosted MCP server.

- A **public server** can be accessed by anyone from any machine. This is useful when you are not exposing sensitive data. It works well for public APIs or services that do not require an API key or token.

![Gram Public Server](/img/blog/the-easiest-way-to-host-mcp-servers/gram-public-server.png)

- An **authenticated server** requires adding your GramAI API key to the project's configuration. This is useful for internal use cases where you want the server to be secured and accessible only to those who have the API key.
